...
class NormativeOrchestrator:
    ...
    def __init__(self, loader, processor, engine, classifier):
        """
        ...
        Аргументы:
            ...
            classifier: Экземпляр класса для предсказания требований (TextClassifier).
    ...
    def run_pipeline(self, url: str) -> pd.DataFrame:
        ...
        if not df.empty:
            #df['lemma_text'] = self.classifier._extract_morph_features(df['text'].tolist())['lemma_text']
            df['predict'] = self.classifier.predict(df['text'].tolist())
